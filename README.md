...
可复用的内容研究与数据工作流技能库，同时是一个可安装的 Claude Code Skill（入口见 [SKILL.md](SKILL.md)）。

这个仓库只放方法、结构、执行规范、通用脚本和 prompt 模板，不放具体项目的原始采集数据，也不放业务项目的正式数据文件。

四种用途：**选题验证**（做产品前验证需求）、**产品来源供给**（为模拟游戏等采集真实素材，交接物是 path_fact 候选）、**产品反馈研究**（爬自己产品的讨论回流设计）、**市场/竞品研究**。详见 SKILL.md。
